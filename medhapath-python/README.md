...
mistake that causes the exact 404 you might have seen). It installs `requirements.txt` and runs
`gunicorn` for you.

**On the free plan specifically**: Render doesn't allow attaching a persistent disk to free
services, so `medhapath.db` lives on the container's normal (writable, but non-persistent)
filesystem. The app works completely normally, but data resets to the seed dataset on every
redeploy and whenever the free service spins down from inactivity and wakes back up. If you need
data to survive permanently, upgrade the Render service to a paid plan and uncomment the `disk:`
block described inside `render.yaml` itself.
